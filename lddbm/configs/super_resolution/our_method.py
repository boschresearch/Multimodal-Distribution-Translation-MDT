...
def load_arguments(parser) -> None:
    ...
    parser.add_argument("--dataset", type=str, default=Datasets.SR.value)
    ...
    parser.add_argument(
        "--training_strategy",
        type=str,
        default=TrainingStrategy.IterativeTraining.value,
    )
    parser.add_argument(
        "--reconstruction_loss_type",
        type=str,
        default=ReconstructionLoss.Predictive.value,
    )
    parser.add_argument(
        "--distance_measure_loss", type=str, default=DistanceMetric.LPIPS.value
    )
    parser.add_argument(
        "--clip_loss_w",
        type=float,
        default=1,
        help="the weight of the clip loss, if 0 no loss",
    )
    ...
    parser.add_argument(
        "--encoder_x_type", type=str, default=Encoders.KlVaePreTrainedEncoder128.value
    )
    parser.add_argument("--decoder_x_path", type=str, default="tmp_decoder_x")
    parser.add_argument(
        "--decoder_x_type", type=str, default=Decoders.KlVaePreTrainedDecoder128.value
    )
    # model
    ...
    parser.add_argument(
        "--encoder_y_type", type=str, default=Encoders.KlVaePreTrainedEncoder16.value
    )
    ...
    parser.add_argument(
        "--w_denoise", type=float, default=1
    )  # weight on the denoiser (DDBM) loss
    ...
    parser.add_argument(
        "--denoiser_type", type=str, default=BridgeModelsTyps.BridgeTransformer.value
    )
