...
class Conv3DEncoder(torch.nn.Module):
    def __init__(
        self, diff_img_size, diff_in_channels, num_views, num_channels, dropout=0
    ):
...
class MV2DEncoder(nn.Module):
    def __init__(self, diff_img_size, diff_in_channels, in_channels=3, dropout=0):
        ...
        self.encoder = nn.Sequential(
            ...
            torch.nn.Dropout(p=dropout),
            nn.Conv2d(
                128, 128, kernel_size=4, stride=2, padding=2
            ),  # -> (256, H/32, W/32)
            torch.nn.SiLU(True),
            torch.nn.Dropout(p=dropout),
            nn.Conv2d(
        ...
        self.fc_encode = nn.Sequential(
            nn.Linear(
                256 * 4 * 4 * 4, 256 * 8 * 8
            ),  # final channel size x final H and W x number of views
        )
    ...
    def forward(self, x):
        ...
        return z.reshape(
            B, self.diff_in_channels, self.diff_img_size, self.diff_img_size
        )
