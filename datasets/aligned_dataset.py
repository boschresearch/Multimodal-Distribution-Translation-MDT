...
class nuScenesDataset(torch.utils.data.Dataset):
    ...
    def sparse_to_dense(self, sparse_occ, occ_shape, t_type='regular'):
        if t_type == 'regular':
            gt = torch.zeros([occ_shape[0], occ_shape[2], occ_shape[3], occ_shape[4]]).to(sparse_occ.device).type(
                torch.float)
    ...
    def __getitem__(self, ix):
        ...
        occ = self.sparse_to_dense(sparse_occ=occ, occ_shape=(1, 0, *(200, 200, 16)), t_type='regular')
