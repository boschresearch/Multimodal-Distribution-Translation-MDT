...
def load_data(
    ...
    if dataset == Datasets.ShapeNet.value:
        ...
        lr_transforms = T.Compose([
            T.Resize((image_size // scale_factor, image_size // scale_factor), Image.BICUBIC),
            T.ToTensor(),
            T.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))  # to scale [-1,1] with tanh activation
        ])
        ...
        valset = CelebaDataset(valid_paths, lr_transforms=lr_transforms, hr_transforms=hr_transforms, train=False)
